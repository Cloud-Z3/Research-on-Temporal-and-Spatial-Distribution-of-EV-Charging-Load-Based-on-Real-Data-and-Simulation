...
for d in district:
    ...
    fripre = [sum([df_fast[j][i] + df_slow[j][i] for i in df_fast.index if df_km['分区'][i] == d]) for j in range(1440, 8640)]
    '''
    fripre2= [sum([fripre[j*60+i] for i in range(60)]) for j in range(24*5)]
    fri=[sum([fripre2[j+i*24] for i in range(5)])/5 for j in range(24)]'''
    fripre2 = [sum([fripre[j + i * 1440] for i in range(5)]) / 5 for j in range(1440)]
    fripre2[0]+=fripre2[-1]
    fripre2[-1]=0
    fri = [sum([fripre2[j*60+i] for i in range(60)]) for j in range(24)]

    #fri=[sum([fripre[j*1440+i] for j in range(5)])/5 for i in range(1440)]
    satpre=[sum([df_fast[j][i]+df_slow[j][i] for i in df_fast.index if df_km['分区'][i]==d]) for j in range(8640,10080)]
    sat=[sum([satpre[j*60+i] for i in range(60)]) for j in range(24)]

    matrix=[]
    for i in range(24):
        matrix.append([i , fri[i], '工作日'])
        matrix.append([i , sat[i], '周  末'])
    df=pd.DataFrame(matrix,columns=['时间(h)','充电需求触发数量(辆)',''])
    ...
    sns.barplot(data=df,x='时间(h)',y='充电需求触发数量(辆)',hue='')
    #fig,ax=plt.subplots(111)
    plt.plot([6,6],[0,max(max(fri),max(sat))],'g--')
    #plt.legend(labels=["工作日","周末"])
    plt.axis([-1,24,0,max(max(fri),max(sat))])
    ...
    plt.ylabel('充电需求触发数量(辆)',fontdict={'size':13})
    #plt.xticks(fontsize=10)
    plt.text(3, max(max(fri),max(sat))/2, s="慢\n充\n模\n式",fontdict=font)
    plt.text(15, max(max(fri),max(sat))/2, s="快\n充\n模\n式",fontdict=font)
    #ax.ticklabel_format(useOffset=False)
    place+=1
plt.savefig('.\\6 DataAna\\ChangeNumWithTime(4 functional areas).png',dpi=500)
plt.show()
#plt.show()
#print(changelow)
'''
